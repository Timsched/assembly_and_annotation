...
# Load the annotation
setwd("/data/users/tschlatter/GenomeandTranscriptome/final")
...
top20_scaff <- fread("/data/users/tschlatter/GenomeandTranscriptome/Assemblies/flye/assembly.fasta.fai", header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
top20_scaff
